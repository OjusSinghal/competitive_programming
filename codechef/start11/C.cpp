...
void run()
{
    ...
    REP(i, 0, n) {
        if (a[i] > 0) {
            int ri = i + 1;
            while (a[(ri % n)] == 0) {
                f[(ri % n)] = min(f[(ri % n)], ri - i);
                ri++;
            }
            int li = i - 1;
            while (a[((li + n) % n)] == 0) {
                f[((li + n) % n)] = min(f[((li + n) % n)], i - li);
                li--;
            }
            f[i] = 0;
        }
    ...
    ll sum = 0;
    REP(i, 0, n) {
        ll left = max(0, k - f[((i - 1 + n) % n)]);
        ll right = max(0, k - f[((i + 1) % n)]);
        sum += ll(a[i] + left + right);
        // cout << i << " " << ll(a[i] + 2 * k - f[((i - 1 + n) % n)] - f[((i + 1) % n)]) << endl;
        // sum += ll(a[i] + 2 * k - f[((i - 1 + n) % n)] - f[((i + 1) % n)]);
    }
